...
The training data `df2` is used to create a model using random forest. We also look at our estimated out of sample error by calling the `modelFit` object. This will return a few stats about our predictor. 

Originally the model used to predict was random forest so we'll start with it.
